...
let $page := 1, $pageSize := 25,
    $nodes_or_dryed := (: data-access:get-all-entries('_qdb-TEI-02'), :) parse-xml(file:read-text('test.xml'))/*/*,
...
return $relevant_nodes_or_dryed(: !(if (. instance of element(util:dryed)) then util:hydrate(., ``[
