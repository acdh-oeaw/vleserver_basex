...
let $esrch-after_updated := esrch:after_updated($data, "`{$dict}`", "`{$db_name}`", "`{$changingUser}`"),
...
return map {
  ...
  'timings': array:join(($esrch-after_updated?timings, $cache-update-after_updated?timings)),
  'memory': map {}
 }]``, map {
