...
let $dict := '_qdb-TEI-02',
    $order := "ascending"
return cache:cache-all-entries($dict)
(: return cache:sort-cache-xquery($dict, ()) :)
(: return cache:get-all-entries($dict, 10000, 25, 'asc',(), cache:count-all-entries($dict)?value) :)
