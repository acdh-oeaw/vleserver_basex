...
declare function _:create_document_list($_self as xs:anyURI, $_embedded_name as xs:string, $_embedded as array(element(json)), $page_size as xs:integer, $total_items as xs:integer, $page as xs:integer, $additional_parameters as map(*), $timings as array(map(xs:string, xs:decimal))?) as element(json) {
(# db:copynode false #) {
  ...
  return
   <json type='object' objects='__links self first last next __embedded timings'>
       ...
       <page>{$page}</page>
       {if (exists($timings)) then <timings type="array">{for $k in distinct-values($timings?*!map:keys(.)) return 
